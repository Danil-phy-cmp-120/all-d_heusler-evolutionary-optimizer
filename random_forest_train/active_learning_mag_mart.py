...
if __name__ == '__main__':
    ...
    for n in range(start_point, X_train.shape[0], step):
        ...
        if (abs(mean_absolute_error - previous_mean_absolute_error) < 0.1 and n > min_step):
        ...
        selected_indices = np.where(absolute_error >= 0.1)[0]
    ...
    ax.set_xlabel(r"$(\mu^{mart}_{pred} - \mu^{mart}_{DFT})/\mu^{mart}_{DFT}$ (%)", size=28, labelpad=3.0)
    train = 100 * (regression.predict(X_train) - y_train) / y_train
    test = 100 * (regression.predict(X_test) - y_test) / y_test
    ax.hist(train[train < 100], bins=15, alpha=0.5, density=True, color="#138A07", label='Training Set')
    ax.hist(test[test < 100], bins=15, alpha=0.5, density=True, color="#bc4749", label='Testing Set')

    # Plot density functions
    density_train = gaussian_kde(train[train < 100])
    density_train.covariance_factor = lambda: 0.25
    ...
    density_test = gaussian_kde(test[test < 100])
    density_test.covariance_factor = lambda: 0.25
    ...
    fig.savefig('histogram_mag_mart_active.png', transparent=False, bbox_inches='tight', dpi=300)
    ...
    plt.grid(True)
    fig.savefig('error_mag_mart_active.png', transparent=False, bbox_inches='tight', dpi=300)
    ...
    fig.savefig('feature_importances_mag_mart.png', transparent=False, bbox_inches='tight', dpi=300)
    print(included[indices][0:10])

    pickle.dump(regression, open('model_mag_mart.pickle', "wb"))
