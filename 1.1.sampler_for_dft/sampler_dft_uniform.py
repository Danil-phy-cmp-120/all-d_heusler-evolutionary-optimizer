...
def generate_uniform_composition_3(element_list, num_points):
    compositions = []
    for i in range(1, num_points + 1):
        for j in range(1, num_points - i + 1):
            k = num_points - i - j + 1
            composition = {
                element_list[0]: i,
                element_list[1]: j,
...
def generate_uniform_composition_4(element_list, num_points):
    compositions = []
    for i in range(num_points + 1):
        for j in range(num_points - i + 1):
            for n in range(num_points - i - j + 1):
                k = num_points - i - j - n
                composition = {
                    element_list[0]: i,
                    element_list[1]: j,
...
def replace_species(structure, composition):
    ...
    site_counts = {el: int(frac * total_sites) for el, frac in composition.fractional_composition.items()}
...
if __name__ == '__main__':
    ...
    uniform_population = generate_uniform_composition_4(element_list, 8)
    for i, element in enumerate(uniform_population):
        print(f'{element} {i+1}/{len(uniform_population)}')
        write_input_vasp(element, element.formula)